...
con la siguiente información que obtenemos a continuación. La cual corresponde a la producción 
de pólizas y reclamos de varios productos asociados al ramo de salud.
...
En la misma información se incluye el deducible $d$ y el monto de máximo de cobertura $M$.
```{r l5t2, echo = TRUE, results = 'asis', out.width='100%', class.source = 'fold-hide'}
claims[ 1:100 ] %>%
  kable(
    label = NA,
    caption = 'Reclamos',
...
Ahora calculemos la duración de cada una de las pólizas, esto es el tiempo de exposición al riesgo
...
Los grupos de riesgo que hemos seleccionado para estudiar la frecuencia y severidad de los reclamos
son los siguientes:
```{r l5t3, echo = TRUE, results = 'asis', out.width='100%', class.source = 'fold-hide'}
grpcl %>%
  kable(
    label = NA,
...
```{r l5g13, out.width='90%', out.height='700px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot( ) +
...
En la lista que hemos creado `fit_count` hemos almacenado cada una de las estimaciones de la 
frecuencia por maximización de verosimilitud para cada uno de los grupos. A continuación, 
presentamos los resultados de la estimación para la frecuencia.
```{r l5t4, echo = TRUE, results = 'asis', out.width='100%', class.source = 'fold-hide'}
estim_frec %>%
  kable(
    label = NA,
...
por tal razón es de esperar que la función de distribución acumulada de los reclamos deducidos tenga 
...
Por no incrementar la complejidad del ejemplo, asumiremos que los reclamos $X$ por cada grupo son 
i.i.d. y estos siguen como distribución alguna de la familia log-normal, es decir 
$X \rightsquigarrow LN( \mu, \sigma )$. Además, es de notar que el razonamiento es válido para 
cualquier otra distribución. Para cada reclamo $i$-ésimo sabemos que se aplicaron un deducible 
$d_i$ y un monto máximo asegurado $M_i$ determinado por la póliza de la cobertura y por la forma del 
deducible $D$, la distribución de probabilidad para los valores aleatorios ya deducidos $Z = D(X)$, 
tiene la forma:
$$
F( z ) = \mathbf{1}_{[0,M_i)}( z ) F_X( z + d_i ) + \mathbf{1}_{[M_i,+\infty)}( z )
$$

Entonces, en este caso en particular la función de verosimilitud logarítmica toma la siguiente forma:
$$
\begin{eqnarray*}
\ell( \mu, \sigma )
...
Preparamos la función de verosimilitud, en este caso debemos definirla explícitamente, sin la ayuda
de alguna función en un paquete, esto se debe a que el deducible $d$ y el monto máximo de cobertura 
$M$ varían entre pólizas.
```{r l5c19}
Fd <- plnorm
fd <- dlnorm
...
for ( g in grpcl$grp ) {
  ...
  loglik_max[[ g ]] <- optim(
    ...
    fn = lglk,
    method = 'BFGS',
    control = list( maxit = 1000, temp = 500, trace = TRUE, REPORT = 100 ) )
  
  estim_sev <- rbind( 
    estim_sev, 
    data.table( grp = g, 
                n = nrow( datsev ),
                meanlog = loglik_max[[ g ]]$par[ 1 ], 
                sdlog = loglik_max[[ g ]]$par[ 2 ],
                loglik = -loglik_max[[ g ]]$value )
  )
}
...
reclamos, lo sumarizamos en la tabla a continuación.
```{r l5t5, echo = TRUE, results = 'asis', out.width='100%', class.source = 'fold-hide'}
estim_sev %>%
  kable(
    label = NA,
...
```{r l5g14, out.width='90%', out.height='700px', fig.align='center', class.source = 'fold-hide'}
xlim <- c( 0, 1e3 )
...
fx <- sapply( x, fd, meanlog = estim_sev$meanlog[ 1 ], sdlog = estim_sev$sdlog[ 2 ] )
dfx <- estim_sev[ , list( x = x, fx = sapply( x, fd, meanlog = meanlog, sdlog = sdlog ) ), by = list( grp ) ]
...
$$
$$
\E\left[ S_g \right] = n_g \E\left[ N_g \right] \E\left[ X_g \right]
= `r formatC( 0, digits = 4, format = 'f' )`
$$
$$
\E\left[ S \right]
= \sum\limits_{g \in G} \E\left[ S_g \right] 
= \sum\limits_{g \in G} n_g \E\left[ N_g \right] \E\left[ X_g \right]
= `r formatC( 0, digits = 4, format = 'f' )`
$$
...
```{r l5g15, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
...
```{r l5g16, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
...
```{r l5g17, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
FeSdat <- NULL
for ( j in 1:p ) {
