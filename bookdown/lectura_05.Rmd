...
Los términos integrales asociados a la severidad presentan un verdadero reto estadístico y 
...
Al tener un número determinado de reclamos $n \in \mathbb{N}$, la distribución de probabilidad del
total de reclamos $S$ puede ser calculada de una forma maś sencilla.
\begin{eqnarray*}
F_S( x )
& = & F_{X_1} \star \cdots \star F_{S_n} ( s )\quad \text{sin $\{X_i\}$ son independientes} \\
...
Una posible estrategias para estimar $F_S$ o $f_S$ es utilizar la transformada de Fourier 
$\mathcal{F}$, la cual convierte las convoluciones en productos y luego invertir de nuevo la 
transformada de Fourier.
\begin{eqnarray*}
F_S 
& = & \mathcal{F}^{-1}\left( \mathcal{F}\left( F_S \right) \right) \\
& = & \mathcal{F}^{-1}\left( \mathcal{F}\left( F_{X_1} \star \cdots \star F_{X_n} \right) \right)
\quad \text{sin $\{X_i\}$ son independientes} \\
& = & \mathcal{F}^{-1}\left( \prod\limits_{i=1}^n \mathcal{F}\left( F_{X_i} \right) \right) \\ 
& = & \mathcal{F}^{-1}\left( \mathcal{F}\left( F_{X} \right)^n \right)
