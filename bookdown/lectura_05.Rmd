...
set.seed( 432147 )
W <- lapply( 1:m, FUN = function( n ) rexp( N, rate = lambda ) )
Tn <- lapply( W, FUN = cumsum )
Tnmax <- max( sapply( Tn, FUN = max ) )
t <- seq( 0, Tnmax, length = 100 )
N <- lapply( Tn, FUN = function( Tk ) sapply( t, FUN = function( t )  sum( Tk <= t ) ) )
Nmax <- max( sapply( N, FUN = max ) )
```

```{r l5g2, out.width='80%', out.height='500px', fig.align='center', class.source = 'fold-hide'}
Ndat <- NULL
for ( j in 1:length( N ) ) {
  Ndat <- rbind( Ndat, data.table( t = t, m = as.factor( j ), N = N[[ j ]] ) )
}
cols <- wes_palette( length( N ), name = "GrandBudapest2", type = "continuous")

EN <- lambda * t
EeN <- Ndat[ , list( EN = mean( N ) ), by = list( t ) ]$EN
time <- t 
