...
N <- n - 1
dx <- diff( x )
fX <- sapply( x[-N], FUN = function( y ) do.call( dist, c( y, pardist ) ) )

fS <- p0
K <- 1
p1 <- ( a + b ) * p0
for ( k in 2:n ) {
  fS <- c( fS, p1 * fX[ k ] + sum( ( a + b * x[ K ] / x[ k ] ) * fX[ K ] * fS[ rev( K ) ] * dx[ K ] ) )
  K <- c( K, k )
}
...
El algoritmo de Panjer suele tener el problema de ajustarse en los primero valores de la 
densidad $f_S$, tal como lo muestra el gráfico a continuación. Incluso este efecto no se resuelve
por completo al aumentar el refinamiento de la malla para aproximar la integral.
```{r l5g6, out.width='80%', out.height='500px', fig.align='center', class.source = 'fold-hide'}
fmax <- 0.03 * max( fS, na.rm = TRUE )

plt <- ggplot( ) +
  geom_histogram( aes( S, after_stat( density ) ), fill = 'dodgerblue4', colour = 'white', alpha = 0.6, bins = 50 ) +
  geom_line( aes( x = x, y = fS ), colour = 'purple3', linewidth = 1 ) +
  scale_x_continuous( breaks = seq( 0, xmax, length = 7 ),
                      labels = formatC( seq( 0, xmax, length = 7 ), digits = 2, format = 'f' ), 
                      limits = c( 0, xmax ), 
                      expand = c( 0, 0 ) ) +
...
```{r l3g2, out.width='80%', out.height='400px', fig.align='center', class.source = 'fold-hide'}
e <- fS - fSx
eb <- max( abs( e ) )

plt <- ggplot() +
  geom_line( aes( x = s, y = fSx ), colour = 'darkred', size = 1 ) + 
...
esperarse son funciones de escalera, siempre crecientes, que toma solo valores positivos.
