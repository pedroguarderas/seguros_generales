...
= \{T_n \leq \} \cap \{ T_{n+1} \leq t\}^c
= \{T_n \leq \} \setminus \{ T_{n+1} \leq t\}
$$
...
2. Se selecciona el número de simulaciones de tiempos de arribo $N \in \N$,

3. Dada la distribución de los tiempos entre arribos $F_W$ para cada simulación $i \in \{1, \ldots, m\}$
se genera una muestra $W_{i,1}, \ldots, W_{i,N}$ de tamaño de $N$,

4. Para cada simulación $i \in \{1, \ldots, m\}$ y cada $n \in \{0, \ldots, N\}$ calculalos los
tiempos de arribo
$$
T_{i,n} = \sum\limits_{k=1} W_{i,k}
$$
y el tiempo inicial es tomado $T_{i,0} = 0$
...
= \sum\limits_{n=0}^{N} \mathbf{1}_{(-\infty,t_l]}\left( T_n \right)
$$
precisamente para cada simulación $i$, el término $N_{i,j}$ es una aproximación al proceso 
...
$T_n = W_1 + \cdots + W_n \rightsquigarrow Gamma( n, \lambda )$ o también $F_W^{\star n}$ es una 
$Gamma( n, \lambda )$. Así por tanto, el proceso estocástico de conteo 
...
set.seed( 432147 )
W <- lapply( 1:m, FUN = function( n ) rexp( Nm, rate = lambda ) )
Tn <- lapply( W, FUN = cumsum )
Tnmax <- max( sapply( Tn, FUN = max ) )
t <- seq( 0, Tnmax, length = nt )
...
Smax <- max( sapply( S, FUN = function( Si ) Si[ Nm ] ) )
```

```{r l5g2, out.width='80%', out.height='500px', fig.align='center', class.source = 'fold-hide'}
Ndat <- NULL
for ( j in 1:m ) {
  Ndat <- rbind( Ndat, data.table( t = t, m = as.factor( j ), N = N[[ j ]], S = S[[ j ]] ) )
}
cols <- wes_palette( m, name = "GrandBudapest2", type = "continuous")

EN <- lambda * t
EeN <- sapply( 1:nt, FUN = function( l ) mean( sapply( 1:m, function( i ) N[[ i ]][ l ] ) ) )
time <- t 

plt <- ggplot( ) +
  geom_step( data = Ndat, aes( x = t, y = N, group = m, colour = m ), size = 0.25 ) +
  ...
  scale_y_continuous( breaks = seq( 0, Nmax, length = 11 ), 
                      formatC( seq( 0, Nmax, length = 2 ), digits = 2, format = 'f' ), 
...
ES <- EN * exp( u + 0.5 * s^2 )
EeS <- sapply( 1:nt, FUN = function( l ) mean( sapply( 1:m, function( i ) S[[ i ]][ l ] ) ) )
Sqmax <- quantile( sapply( S, FUN = function( Si ) Si[ Nm ] ), probs = 0.99 )

plt <- ggplot( ) +
  geom_step( data = Ndat, aes( x = t, y = S, group = m, colour = m ), size = 0.25 ) +
