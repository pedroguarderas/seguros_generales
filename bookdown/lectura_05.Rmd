...
set.seed( 432147 )
W <- lapply( 1:m, FUN = function( n ) rexp( N, rate = l ) )
...
```{r l5g2, out.width='80%', out.height='400px', fig.align='center', class.source = 'fold-hide'}
Ndat <- NULL
for ( j in 1:length( N ) ) {
  Ndat <- rbind( Ndat, data.table( t = t, m = as.factor( j ), N = N[[ j ]] ) )
}
cols <- wes_palette( length( N ), name = "Rushmore", type = "continuous")

plt <- ggplot( data = Ndat ) +
  geom_step( aes( x = t, y = N, group = m, colour = m ), size = 0.25 ) + 
