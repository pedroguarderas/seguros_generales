...
En la lista que hemos creado `fit_count` hemos almacenado cada una de las estimaciones por 
maximización de verosimilitud para cada uno de los grupos. Ahora creamos un objeto que sumariza 
...
estim_frec %>%
  kable(
    ...
    row.names = FALSE,
    col.names = c( '$g$', '$\\alpha$', '$p$', '$\\E[ N ]$', '$\\ell( n, \\mu )$', '$AIC$', '$BIC$' ),
    align = 'lrrrrrr',
...
valores de los reclamos ya viene censurados, por la función deducible $D$, por tal razón es 
de esperar que la función de distribución acumulada de los reclamos deducidos tenga singularidades.
Si buscamos utilizar el método de máxima verosimilitud para la estimación de la mejor distribución
debemos hacer uso de la forma extendida \@ref(eq:loglik).
```{r l5c18}
# prod_prep[ , ER := interval( fi, fe ) / dyears( 1 ) ]
# prod_prep <- prod_prep[ , list( ER = sum( ER, na.rm = TRUE ) ), by = list( sex, xg, di, de ) ]
# setorder( prod_prep, sex, xg, di )
...
for ( g in grpcl$grp ) {
  ...
  lglk <- function( theta ) {
    ...
    l <- sum( sapply( datsev[ claim != amount ]$claim,
                      FUN = function( x ) log( fd( x, meanlog = theta[1], sdlog = theta[2] ) ) ) )

    # suma en las discontinuidades
    sg <- datsev[ claim == amount ]$claim

    l <- l + ifelse( length( sg ) > 0,
                     sum( sapply( sg, FUN = function( x ) log( 1 - Fd( x, meanlog = theta[1], sdlog = theta[2] ) ) ) ),
                     0 )
...
plt <- ggplot( ) +
  geom_histogram( data = sev_claims, aes( claim, after_stat( density ) ), 
                  fill = 'olivedrab3', colour = 'olivedrab4', alpha = 0.3, bins = 30 ) +
  facet_grid( sex ~ xg, scales = "free" ) +
  xlab( TeX( "$D(X)$" ) ) + 
  ylab( TeX( "$f_{D(X)}$" ) ) + 
  theme_bw() +
  theme( legend.position = "none" )
