...
```{r l4g1, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot() +
...
```{r l4g2, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot() +
  geom_point( aes( x = 1:n, y = v ), colour = 'darkred' ) + 
  xlab( TeX( "$k$" ) ) + 
  ylab( TeX( "$k \\frac{p_k}{p_{k-1}}$" ) ) + 
...
```{r l4g3, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot() +
  geom_point( aes( x = 1:n, y = v ), colour = 'darkred' ) + 
  xlab( TeX( "$k$" ) ) + 
  ylab( TeX( "$k \\frac{p_k}{p_{k-1}}$" ) ) + 
...
```{r l4t1, echo = TRUE, results = 'asis', out.width='100%', class.source = 'fold-hide'}
...
```{r l4g4, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot() +
...
::: {.definition #destinsesg  name="Estimador más eficiente"}
...
$$
P_{\tau,\theta}( A ) = P_\theta\left( \tau\left(X_1, \ldots,X_n\right)^{-1}( A ) \right)
$$
...
$$
\E_{P_{\tau,\theta}}\left[ h\left( \tau\left( X_1,\ldots, X_n \right) \right) \right] = 0 
\Longrightarrow P_\theta\left( h\left( \tau\left( X_1,\ldots, X_n \right) \right) = 0 \right) = 1
$$
...
$$
\widehat{\theta} 
= \underset{\theta \in \Theta}{\argsup} \sum\limits_{i=1}^n \log P_{\theta} \left( B_i \right)
$$
...
Podemos tener una variable aleatoria discreta $N$, de la cual hemos observado una muestra de 
$N_1, \ldots, N_m$ asumiendo independencia entre cada una de las observaciones. Podemos ademas 
suponer que $N$ pertenece a la familia de distribuciones discretas en la clase $(0,a,b)$ y por tanto
$P( N = 0 ) = p_0$ es un parámetro inicial $p_n = ( a + b/n) p_{n-1}$, para cualquier
$n > 1$.
$$
P\left( N_1 = n_1 \land \cdots \land N_m = n_m \right)
= \prod\limits_{i=1}^m p_i^{n_i}
$$
