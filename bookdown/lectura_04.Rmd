...
$$
  5. La función $F$ es a variación acotada. Esto quiere decir que existe un real $C > 0$, para 
  cualquier partición $\{x_i\}_{i\in\Z}$, tal que:
...
están bien definidas, de donde resulta la siguiente igualdad.
$$
\begin{eqnarray*}
\E\left[ X \right]
& = & \int\limits_{\R} x dF( x ) \\
...
<!------------------------------------------------------------------------------------------------->
::: {.definition #dexpcond name="Varianza condicionada"}
...
$\left\| F_n - F \right\| = \underset{x \in \R}{\sup} \left| F_n( x ) - F( x ) \right|$ tenemos la
siguiente igualdad que precisamente expresa la convergencia en casi todo punto.
$$
P\left( \underset{n \rightarrow +\infty}{\lim} \left\| F_n - F \right\| = 0 \right) = 1
$$
:::
...
```{r l4t1, echo = TRUE, results = 'asis', out.width='100%' }
...
```{r l4g5, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
e <- f - IFf
