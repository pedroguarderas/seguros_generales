...
1. si $z > a$, entonces $F( z ) = 1$

2. si $z < a$, entonces $F( z ) = F_X( z )$

3. si $z = a$, entonces $F( a ) = 1$

4. la función $F$ es discontinua en $a$
$$
\underset{z \nearrow a}{\lim} F( z ) = \underset{z \nearrow a}{\lim} F_X( z ) = F_X( a )
$$
por otra parte
$$
\underset{z \searrow a}{\lim} F( z ) = \underset{z \searrow a}{\lim} \mathbf{1}_{[a,+\infty)}( z )
...
```{r l3t1, echo = TRUE, results = 'asis', out.width='80%', tab.cap='Error versus número de simulaciones' }
