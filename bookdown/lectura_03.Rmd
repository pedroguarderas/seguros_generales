...
Considerando una variable aleatoria discreta $K : \Omega \longrightarrow D \subset \mathbb{R}$, 
...
3. La esperanza de una constante $a \in \mathbb{R}$ es la misma constante.
\begin{equation}
...
consideraremos que son i.i.d. y con media común finita $\mathbb{E}[X_i] = \mu < +\infty$. Entonces,
...
\begin{equation}
\underset{n \rightarrow +\infty}{\lim} P\left( \left| \sum\limits_{i=1}^n X_i - \mu \right|\right) = 0
\end{equation}
:::
...
$\mathbb{E}[X_i] = \mu < +\infty$ y varianza común finita
...
dt %>%
  ...
  scroll_box( width = "1050px", height = "500px" )
```
...
cualquier $\varepsilon \in \mathbb{R}$.
\begin{equation}
...
que además tiene varianza finita $\mathbb{V}[X] < +\infty$, entonces si tomamos un valor 
$\rho \in [0,1]$, se satisface la siguiente desigualdad:
\begin{equation}
P( X \geq \rho \mathbb{E}[ X ] ) \geq ( 1 - \rho )^2 \frac{\mathbb{E}[ X ]^2}{\mathbb{E}[ X^2 ]}
\end{equation}
:::
...
La **transformada de Fourier** para una función integrable $f : \mathbb{R}^n \longrightarrow \mathbb{R}$, 
está definida como:
\begin{equation}
\mathcal{F}( f )( \omega ) = 
\int\limits_{\mathbb{R}^n} f( x ) \exp( -2 \pi i \omega \cdot x )\ dx
\end{equation}

la inversa de la transformada de Fourier, para una función $g : \mathbb{R}^n \longrightarrow \mathbb{R}$,
está dada por:
\begin{equation}
\mathcal{F}^{-1}( g ) = 
\int\limits_{\mathbb{R}^n} g( x ) \exp( 2 \pi i \omega \cdot x )\ dx
\end{equation}

se satisface la siguiente igualdad
\begin{equation}
\mathcal{F}^{-1}\left( \mathcal{F}( f ) \right) = f
\end{equation}

Además, la transformada de Fourier satisface las siguientes propiedades:

1. La transformada de Fourier es lineal, si $a,b \in \mathbb{R}$ y 
$f :\mathbb{R}^n \longrightarrow \mathbb{R}$ y $g :\mathbb{R}^n \longrightarrow \mathbb{R}$, funciones
integrables.
\begin{equation}
\mathcal{F}\left( a f + b g \right) = a \mathcal{F}( f ) + b \mathcal{F}( g )
\end{equation}

2. La transformada de Fourier de la convolución de funciones es el producto de las transformadas de 
Fourier.
\begin{equation}
\mathcal{F}\left( f \star g \right) = \mathcal{F}( f ) \mathcal{F}( g )
\end{equation}
:::
...
valores discretos $x_k = a + k h$, con $h = \frac{b-a}{N}$ y $k \in \{0, \ldots, N\}$. Así, tenemos 
la siguiente aproximación a la transformada de Fourier.
\begin{eqnarray*}
\mathcal{F}( f )( \omega )
& = & \int\limits_{\mathbb{R}} f( x ) \exp( -2\pi i \omega x )\ dx \\
...
puede considerar $\omega_j = \frac{j}{b-a}$
\begin{eqnarray*}
\mathcal{F}( f )\left( \omega_j \right) 
& = & h \exp\left( -2\pi i \frac{j}{b-a} a \right) 
\sum\limits_{k=0}^{N-1} f( x_k ) \exp\left( -2\pi i \frac{jk}{N} \right) \\
...
alpha <- 2
theta <- 3
b <- qgamma( 0.999, shape = alpha, scale = theta )
a <- 0
h <- ( b - a ) / N
n <- 0:N
x <- a + n * h
...
\begin{equation}
\left\| \{f( x_k ) \} - 
DFT^{-1}\left[ \left\{ \frac{1}{h} \exp\left( 2\pi i \frac{j}{b-a} a \right) \hat{f}_j \right\} \right] \right\|_2
...
```{r l2g1}
plot( x, f, type = 'l' )
points( x, Re( IFf ), col = 'green', pch = 16, cex = 0.5 )

e <- f - IFf
e <- e / sd( e )
hist( e, breaks = 40, xlim = c( -6, 6 ) )
```

<!------------------------------------------------------------------------------------------------->
### Uso de la transformada de Fourier

Esta aproximación es realmente de utilidad cuando se tiene que determinar la densidad de la 
suma $S_n = \sum\limits_{i=1}^n X_i$ de varias variables aleatorias 
...
\end{equation}
sin embargo esta convolución implica el realizar un interación en $n$-dimensiones.

Con el uso de la transformada de Fourier sabemos por la independencia entre las variables $\{X_i\}$
que $\mathcal{F}\left( f_S \right) = \prod\limits_{i=1}^n \mathcal{F}\left( F_{X_i} \right)$. Por otra 
parte, con lo anterior sabemos que es posible aproximar numéricamente cada $\mathcal{F}\left( X_i \right)$ 
con una serie $\{\hat{f}_{i,j}\}$ dada por la discretización de la transformada de Fourier y su aplicación 
sobre la discretización $f_{i,k} = f_{X_i}( s_k )$. Por tanto, para cada $i \in \{1, \ldots, n\}$, 
se puede calcular de forma separada las aproximaciones a cada una de las transformadas
\begin{equation}
\mathcal{F}( f_{X_i} )\left( \omega_j \right)  
\approx \hat{f}_{i,j} 
= h \exp\left( -2\pi i \omega_j a \right) \left( DFT\left[ \{f_{i,k}\} \right]  \right)_j
\end{equation}

con lo anterior, se puede también realizar una aproximación a la trasformada de Fourier 
$\mathcal{F}\left( f_S \right)$ de la densidad de probabilidad $f_S$, como el producto de sus 
transformadas de Fourier.
\begin{equation}
\mathcal{F}( f_{S} )\left( \omega_j \right)  
= \prod\limits_{i=1}^n \mathcal{F}\left( F_{X_i} \right)\left( \omega_j \right)
\approx \prod\limits_{i=1}^n \hat{f}_{i,j}
\end{equation}

finalmente utilizando la inversión de la transformada de Fourier discreta podemos calcular una 
serie que precisamente aproxima a la densidad $f_S$
\begin{equation}
\{ f_{S}( s_k ) \}
\approx DFT^{-1}\left[ \left\{ \frac{1}{h} \exp\left( 2\pi i \omega_j a \right) 
\prod\limits_{i=1}^n \hat{f}_{i,j} \right\} \right]
\end{equation}

```{r l2c9}
N <- 7000
alpha <- c( 2, 3, 5, 7, 9, 11, 1, 3, 5, 9 )
theta <- 3
...
fS <- Re( IFfS )
fSx <- sapply( s, FUN = function( sk ) dgamma( sk, shape = sum( alpha ), scale = theta ) )
```

```{r l2g2}
plot( s, fSx, type = 'l' )
points( s, fS, col = 'green', pch = 16, cex = 0.5 )
...
por una tasa constante $i \in \mathbb{R}$ en el tiempo, es decir, la función de actualización toma 
la forma 
\begin{equation}
v(s,t) = ( 1 + i )^{-(t-s)}
\end{equation}
