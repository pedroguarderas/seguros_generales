...
es de notar que numérica por la aritmética en coma flotante la expresión $\DFT^{-1}\left[ \left\{ \frac{1}{h} \exp\left( 2\pi i \frac{j}{b-a} a \right) \hat{f}_j \right\} \right]$ puede tener parte compleja 
muy pequeña, cercana a $0$. Para superar este posible problema numérico tomamos solo la parte real.
\begin{equation}
\{f( x_k )\} 
\approx \operatorname{Re}\left( \DFT^{-1}\left[ \left\{ \frac{1}{h} \exp\left( 2\pi i \frac{j}{b-a} a \right) 
...
e <- f - IFf
e <- e / sd( e )
hist( e, breaks = 40, xlim = c( -6, 6 ), probability = TRUE )
```
...
que $\Fo\left( f_S \right) = \prod\limits_{i=1}^n \Fo\left( F_{X_i} \right)$. Por otra 
parte, con lo anterior sabemos que es posible aproximar numéricamente cada $\Fo\left( X_i \right)$ 
...
$\Fo\left( f_S \right)$ de la densidad de probabilidad $f_S$, como el producto de sus 
