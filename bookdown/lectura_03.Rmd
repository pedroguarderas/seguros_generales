...
<!------------------------------------------------------------------------------------------------->
::: {.definition #em name="Esperanza de una variable aleatoria"}
...
<!------------------------------------------------------------------------------------------------->
::: {.definition #em name="Varianza de una variable aleatoria"}
...
aleatoria $X$. Si $x,y \in \R$ son dos puntos de continuidad de la distribución $F$, then
$$
...
$$
F( x \mid A ) = P\left( X \leq x \middle| A \right)
$$
Se satisface algunas propiedades:
...
$P\left( X \leq x \middle| \Omega \right) \leq P( A )$, entonces $0 \leq F(x\mid A) \leq 1$,

4. Para cualquier $A \in \F$, la función de distribución acumulada condicionada $F( \cdot \mid A )$ es no decreciente,

5. Para cualquier $A \in \F$, la función de distribución acumulada condicionada $F( \cdot \mid A )$ es continua por derecha,

En algunos casos la distribución acumulada de la variable aleatoria $X$ se nota por $F_X$ y la 
distribución acumulada de $X$ condicionada por el evento $A$, se nota por $F_{X\mid A}$.
:::
...
<!------------------------------------------------------------------------------------------------->
::: {.definition #dexpcond name="Esperanza condicionada"}
...
= \int\limits_{A} X( \omega )\ dP( \omega \mid A )
= \int\limits_{\R} x\ dF( x \mid A )
$$
cuando $F(x\mid A)$ es derivable, con densidad de probabilidad $f(x|\mid A)$, la esperanza 
condicionada también se puede escribir como:
$$
\E\left[ X \middle| A \right] 
= \int\limits_{\R} x\ f( x \mid A )\ dx
$$
...
::: {.definition #dlawtotvar name="Ley de la varianza total"}
Consideremos dos variables aleatorias $X, Y$, ambas a valores reales, entonces se satisface la 
siguiente igualdad:
$$
\V[ Y ] = \E\left[ \V\left[ Y \middle| X \right] \right] + 
\V\left[ \E\left[ Y \middle| X \right] \right]
$$
:::
...
& = & F( x \mid n ) p_n\qquad \text{simplificando notación} \\
\end{eqnarray*}
$$
...
& = & \sum\limits_{n \in \mathbb{N}} F( x \mid n ) p_n 
\end{eqnarray*}
$$
...
```{r l3g1, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot() +
...
```{r l3g2, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
dat <- NULL
for ( k in 1:n ) {
...
```{r l3g3, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
plt <- ggplot() +
...
```{r l3g4, out.width='80%', out.height='450px', fig.align='center', class.source = 'fold-hide'}
e <- f - IFf
eb <- max( abs( e ) )
