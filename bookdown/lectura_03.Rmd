...
La distribución acumulada de la variable aleatoria correspondiente al reclamo total $S$, para el
caso del modelo individual toma la forma:
\begin{eqnarray*}
F_S( s ) 
& = & P( S \leq s ) \\
& = & P\left( \sum\limits_{i=1}^n X_i \leq s \right) \\
& = & F_{X_1} \star \cdots \star F_{X_n}( s )\quad \text{distribución de la suma de variables aleatorias} \\
& = & F^{\star n}_{X}( s ) \quad \text{si $\{X_i\}$ son idénticamente distribuidas}
\end{eqnarray*}

Es sencillo determinar que la densidad de probabilidad de $S$, está dado por el producto de convolución
\begin{eqnarray*}
f_S( s ) & = & f_{X_1} \star \cdots \star f_{X_n}( s )\quad \text{densidad de la suma de variables aleatorias} \\
& = & f^{\star n}_{X}( s ) \quad \text{si $\{X_i\}$ son idénticamente distribuidas} 
\end{eqnarray*}
...
Es de notar que una algoritmo como el descrito tienen una falencia cuando la frecuencia de siniestros
es poco observada, esto sucede cuando $P( N = 0 )$ es alta y por tal razón para tener cálculos con 
una buena aproximación mediante el uso de simulaciones se necesitará un gran número de las mismas.
