...
está definida como:
\begin{equation}
\mathcal{F}( f )( \omega ) = \frac{1}{(2\pi)^{\frac{n}{2}}} 
\int\limits_{\mathbb{R}^n} f( x ) \exp( -i \omega \cdot x )\ dx
\end{equation}

la inversa de la transformada de Fourier, para una función $g : \mathbb{R}^n \longrightarrow \mathbb{R}$,
está dada por:
\begin{equation}
\mathcal{F}^{-1}( g ) = \frac{1}{(2\pi)^{\frac{n}{2}}} 
\int\limits_{\mathbb{R}^n} g( x ) \exp( i \omega \cdot x )\ dx
\end{equation}
...
Con el suso de la transformada de Fourier sabemos por la independencia entre las variables $\{X_i\}$
que $\mathcal{F}\left( f_S \right) = \prod\limits_{i=1}^n \mathcal{F}\left( F_{X_i} \right)$. Por otra 
parte, con lo anterior sabemos que es posible aproximar numéricamente cada $\mathcal{F}\left( X_i \rigt)$ 
...
con lo anterior, se puede también realizar una aproximación a la trasformada de Fourier $\mathcal{F}[f_S]$ 
de la densidad de probabilidad $f_S$, como el producto de sus transformadas de Fourier.
\begin{equation}
