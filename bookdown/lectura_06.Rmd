...
aleatorias que siguen una distribución $Gamma( \alpha, \sigma )$ y un número de $n = 5000$ reclamos. 
...
FS <- function( x ) pgamma( x, shape = n * sh, scale = sigma )

C <- 0.7655 + 8 * ( 1 + exp(1) ) # constante universal del teorema
f <- function( x ) ( abs( x - EX )^3 ) * dgamma( x, shape = sh, scale = sigma )
tau <- integrate( f = f, lower = 0, upper = Inf )$value / sqrt( VX )^3

s <- seq( 0, 26e6, length = 5000 )
Fms <- sapply( s, FUN = FS )
...
Esto es válido siempre y cuando el error de aproximación 
