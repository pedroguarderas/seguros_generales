...
central \@ref(thm:tlc), para ello se requiere un gran número de elementos en la suma que se 
consideran en los reclamos totales $S$, como ya lo muestra la desigualdad \@ref(thm:beress), 
la convergencia requiere un número de términos inversamente proporcional al cuadrado del error
de aproximación. Sin embargo, si en caso el número de reclamos $n$ no es lo suficientemente como 
para un obtener un error de aproximación deseado, podemos hacer uso de un resultado más fuerte como 
...
# Simulación de reclamos totales
m <- 1e3
S <- sapply( 1:m, FUN = function( i ) sum( rgamma( n = n, shape = sh, scale = sigma ) ) )
...
número de siniestros $n$ si disminuye el intervalo, proporcionando una mejor estimación de la 
...
la acotación igual sería válida.
:::
