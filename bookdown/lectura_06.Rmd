...
Un modelo puede estar equilibrado financieramente $\mathbb{E}[R] = 0$, pero se desconoce la 
...
\begin{equation}
  \rho( S ) = \mathbb{E}[S] \approx \overline{S}
\end{equation}

2. Prima de riesgo con recargo sobre la esperanza matemática
\begin{equation}
  P = \rho( S ) = (1 + \rho) \mathbb{E}[S] \approx (1 + \rho) \overline{S}
\end{equation}
...
\begin{equation}
  P = \rho( S )
  = \frac{1}{2} \mathbb{E}\left[\exp(tS)\right]
...
unidades aseguradas está dado por $n \in \mathbb{N}$. Utilizaremos el modelo individual para
...
Es de notar que cada variable aleatoria $X_i$ tiene un parámetro diferente $\alpha_i$. 

Estudiemos la variable $S$ utilizando un algoritmo de simulación, para así calcular algunas primas
en las diferentes formas que hemos citado. Primeramente definamos los parámetros
```{r l5c3}
...
las experanzas de $\mathbb{E}[X_i]$, para cada $i \in \{1,\ldots,n\}$ y de igual forma podemos 
calcular la esperanza del reclamo total $\mathbb{E}[S] = \sum\limits_{i=1}^n \mathbb{E}[X_i]$. 
Además como asumimos independencia entre los $X_i$, la varianza de $S$ también puede ser calculada
fácilmente como $\mathbb{V}[S] = \sum\limits_{i=1}^n \mathbb{V}[X_i]$.
```{r l5c4}
EX <- a * theta
VX <- a * theta^2
ES <- sum( EX )
...
La variable aleatoria del reclamo total $S$ la podemos simular, tomando una muestra de tamaño $m$, 
i.e. $S_1,\ldots,S_m$.
```{r l5c5}
m <- 1e5
