...
Como ya lo mencionamos en este ejemplo la densidad de probabilidad de la severidad $S$ resulta de la
...
de reclamos dados por una variable aleatoria discreta $N_i$, cada uno de las $i$ unidades puede 
presentar un número de reclamos $X_{i,1}, \ldots, X_{i,N_i}$. De agregar el total de reclamos, 
...
y reclamos, entonces el reclamos total anterior puede ser visto como un modelo colectivo de pérdida.
$$
S 
...
central \@ref(thm:tlc), para ello se requiere un gran número de elementos en la suma determina 
los reclamos totales $S$, como ya lo muestra la desigualdad \@ref(thm:beress), 
la convergencia requiere un número de términos inversamente proporcional al cuadrado del error
de aproximación. Sin embargo, si en caso el número de reclamos $n$ no es lo suficientemente grande 
como para obtener un error de aproximación deseado, podemos hacer uso de un resultado más fuerte como 
la desigualdad de concentración de \@ref(thm:beress), la misma no solo implica el teorema del límite 
central, sino que además proporciona una acotación y velocidad de convergencia conforme se aumenta 
$n$. Esta desigualdad puede ser de gran utilidad para tener una acotación de la distribución de los 
reclamos totales, para cualquier valor de $n$.
...
\end{gather*}
cambiando la variable $s = \E[S] + \sqrt{\V[S]} u$. El $\max$ y $\min$ resultan debido a que una
probabilidad siempre está acotada en el rango de $0$ a $1$.
...
Como se puede ver en el gráfico a continuación la acotación funciona muy bien y al aumentar el 
número de siniestros $n$ se disminuye el rango de la acotación, proporcionando así una mejor 
estimación de la distribución de los reclamos totales $S$.
...
lo suficientemente pequeño. Además, es de notar que se está trabajando con valores que solo están 
