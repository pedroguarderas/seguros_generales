...
Una medida de riesgo se dice **coherente** si satisface la condición de homogeneidad positiva, 
monotonía, invarianza ante las traslaciones y sub-aditivadad. 
...
1. Si $g( x ) = \mathbf{1}_{(\alpha,+\infty)}$, entonces $\rho_g( X ) = \VaR_{1-\alpha}( X )$

2. Si $g( x ) = \min\left( \frac{x}{\alpha}, 1 \right)$, entonces $\rho_g( X ) = \TVaR_{1-\alpha}( X )$
...
por inversiones $I = 0$. Esto sobre todo en el contexto donde no se haya acumulado mucha experiencia
en el ramo, un escenario algo más desfavorable.
$$
R = \Pi - S - G - K
$$
...
$$
\begin{split}
\Pi & \geq \frac{1}{1 - \gamma} \left( K + \VaR_{1 - \alpha}(S) \right) \\
\Pi & \geq \frac{1}{1 - \gamma} \left( r \rho(S) - r \E[S] + \VaR_{1 - \alpha}(S) \right)\\
\Pi & \geq \frac{1}{1 - \gamma} \left( ( 1 + r ) \VaR_{1 - \alpha}(S) -  r \E[S] \right)
\end{split}
$$
...
Sin mucha complicación para el caso cuando hay inversiones $I$, el razonamiento llega a ser muy 
similar y la prima de riesgo debería satisfacer una cota como la siguiente:
$$
\begin{split}
\Pi & \geq \frac{1}{1 - \gamma} \left( K - I + \VaR_{1 - \alpha}(S) \right) \\
\Pi & \geq \frac{1}{1 - \gamma} \left( r \rho(S) - r \E[S] - I + \VaR_{1 - \alpha}(S) \right)\\
\Pi & \geq \frac{1}{1 - \gamma} \left( ( 1 + r ) \VaR_{1 - \alpha}(S) - I -  r \E[S] \right)
\end{split}
$$

si incluso las inversiones son proporcionales a la prima $I = \iota \Pi$ se llega a las siguientes
desigualdades.
$$
\begin{split}
\Pi & \geq \frac{1}{1 + \iota - \gamma} \left( K + \VaR_{1 - \alpha}(S) \right) \\
\Pi & \geq \frac{1}{1 + \iota - \gamma} \left( r \rho(S) - r \E[S] + \VaR_{1 - \alpha}(S) \right)\\
\Pi & \geq \frac{1}{1 + \iota - \gamma} \left( ( 1 + r ) \VaR_{1 - \alpha}(S) -  r \E[S] \right)
\end{split}
$$
Pero, en general el retorno de las inversiones no es certero, dependiendo de los instrumentos 
financieros que se utilice, siendo en sí $I$ también otra variable y el criterio de cobertura
$P\left( \Pi + I - S - G - K < 0 \right) < \alpha$ debe tomar en cuenta que tanto $S$ como $I$
son variables aleatorias, eso da una naturaleza estocástica a los dos grupos de desigualdades 
anteriores y deben ser tomadas con mucho cuidado. La interpretación, antes que su uso, es lo 
relevante e importante a tener en cuenta, con lo anterior se muestra que las inversiones pueden 
contribuir a la generación de una prima de riesgo más competitiva, pero su inclusión exige un modelo 
de riesgo más complejo, una administración entre activos y pasivos más activa y de seguro una 
gestión de riesgos conjunta más demandante.
...
3. Si el riesgo a cubrir está limitado es decir, para la variable del valor total de reclamo $S$
tenemos que $P( S \leq M ) = 1$, para un valor $M > 0$, entonces jamás la prima será superior a $M$
$$
\rho( S ) \leq M
$$
